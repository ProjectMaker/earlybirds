...
cli.responders.help = () => {
    const commands = {
        ...
        'analyse products': 'Call api vision and update all products',
        'analyse product': 'Call api vision and update the given product',
...
cli.responders.analyzeProduct = (productId) => {
    ...
    if (productId) {
        ...
        localStorage.read('products', productId)
            .then(_product => {
                product = _product
                return helpers.callVisionApiColor(`http:${product.photo}`)
            })
            .then(color => {
                product.dominantColors = color
                return localStorage.update('products', productId, product)
            })
            .then(() => console.log('\x1b[36m%s\x1b[0m', `Product dominant color updated`))
            .catch((err) => console.log('\x1b[31m%s\x1b[0m', err))
    } else console.log('\x1b[31m%s\x1b[0m', 'Param productId is invalid')
}
